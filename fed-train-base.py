...
parser.add_argument("-batch", "--Batch_size", default=4, help="Batch Size")
...
def evaluate(model, dataloader):
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            mask = data["mask"].to(device).float()  # 손상된 이미지
...
# FedProx Aggregation: 현재 FedProx의 글로벌 집계 단계는 FedAvg와 동일하지만,
# 로컬 업데이트 시 proximal term을 추가하는 것이 핵심입니다.
# 추후 로컬 업데이트 함수에 proximal term을 추가할 수 있도록 별도 함수로 제공.
def fed_prox(global_model, client_models, client_sample_counts, mu=0):
    """
    FedProx aggregation 함수.
    현재 글로벌 집계 단계는 FedAvg와 동일합니다.
    mu: proximal term의 계수 (추후 로컬 업데이트에 활용 가능)
    """
...
for cid in range(num_clients):
    # 각 클라이언트 폴더 예: "distribute_datasets/c0_dataset"
    client_dir = os.path.join(args.data_dir, f"/home/prml/YY/2503_miccai/datasets/train_datasets_non_iid/c{cid}_dataset/train")
    train_loader = make_client_train_dataloader(client_dir, batch_size=TrainBatchSize, num_workers=4)
    client_train_loaders.append(train_loader)
    client_train_sizes.append(len(train_loader.dataset))

# 중앙집중식 validation 데이터 로더 (예: "dataset/val" 구조)
...
while global_round < global_rounds:
    ...
    for cid in range(num_clients):
        ...
        local_model.train()
        optimizer_local = optim.AdamW(local_model.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.01)
        ...
        for _ in range(local_epochs):
            with tqdm(client_train_loaders[cid], unit="batch", desc=f"Client {cid} Epoch") as tepoch:
                for batch in tepoch:
                    ...
                    outputs = local_model(inputs,mask)
    ...
    # 글로벌 모델 업데이트 (FedAvg 또는 FedProx로 선택 가능)
    # 예를 들어, FedAvg 사용:
    global_model = federated_average(global_model, client_models, client_samples)
    # 또는 FedProx 사용 시 (proximal term 계수 mu 조정 가능):
    # global_model = fed_prox(global_model, client_models, client_samples, mu=0.01)
